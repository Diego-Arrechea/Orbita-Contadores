...
import { facturadoEnVentana, hastaSemestreRecat, type CalculoCliente } from '@/lib/monotributo';
...
export function SituacionActual({ cliente, calc, onVerComprobantes }: Props) {
  ...
  // Evaluación de recategorización (#3): facturado 12m del PERÍODO configurado (semestre + año, global
  // del contador en Configuración → Ventanas) + la categoría que le correspondería. El facturómetro
  // OFICIAL de ARCA es sólo el rolling a hoy, así que este período se calcula por comprobantes (el
  // historial de la ficha trae 26 meses, alcanza para semestres anteriores).
  const hastaEval = hastaSemestreRecat(config.periodoRecat.semestre, config.periodoRecat.anio);
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <Card className="p-5 sm:p-7 col-span-full lg:col-span-2">
        ...
        {/* Evaluación de recategorización (#3): el facturado del PERÍODO configurado (semestre + año,
            en Configuración → Ventanas) y la categoría que le correspondería. El facturómetro oficial
            es sólo rolling a hoy → acá se calcula por comprobantes. El período se ajusta en Config. */}
        {!verInflacion && (
          <div className="mt-5 border-t border-border/60 pt-4">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-2.5 inline-flex items-center gap-1.5">
              <CalendarRange className="h-3.5 w-3.5" />
              Recategorización · {config.periodoRecat.semestre.replace('-', '–')} {config.periodoRecat.anio}
            </div>
            <div
              className={cn(
            ...
            <div className="mt-1.5 text-[11px] text-muted-foreground">
              ...
              . El período se ajusta en Configuración → Ventanas.
            </div>
          </div>
        )}
      </Card>
